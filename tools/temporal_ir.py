...
def main():
    ...
    for idx in range(num):
        for direction in ["irL", "irR"]:
            ...
            pattern = get_smoothed_ir_pattern(diff, ks=args.patch, threshold=0.005)

            # Save extracted IR pattern
            pattern = (pattern * 255).astype(np.uint8)
            cv2.imwrite(
                os.path.join(args.data_folder, p, f"1024_{direction}_real_temporal_ps{args.patch}.png"), pattern
            )
